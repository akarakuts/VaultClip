...
class HistoryItemBaseCellView: NSTableCellView {
    ...
    override func rightMouseDown(with event: NSEvent) {
        ...
        if item.isPassword {
            if table.listMode == .passwords {
                ...
                let copyURL = NSMenuItem(title: L10n.contextCopyURL, action: #selector(contextCopyPasswordURL(_:)), keyEquivalent: "")
                copyURL.isEnabled = !item.passwordURL.isEmpty
                menuItems.append(copyURL)
                let openURL = NSMenuItem(title: L10n.contextOpenURL, action: #selector(contextOpenPasswordURL(_:)), keyEquivalent: "")
                openURL.isEnabled = item.passwordOpenableURL != nil
                menuItems.append(openURL)
    ...
    @objc private func contextCopyPasswordURL(_ sender: NSMenuItem) {
        guard let table = hostTableView, displayedRow >= 0 else { return }
        table.historyDelegate?.historyTableView(table, copyPasswordURLAt: displayedRow)
    }
    
    @objc private func contextOpenPasswordURL(_ sender: NSMenuItem) {
        guard let table = hostTableView, displayedRow >= 0 else { return }
        table.historyDelegate?.historyTableView(table, openPasswordURLAt: displayedRow)
    }
