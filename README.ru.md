...
Сохранённые записи: комментарий, логин, необязательный URL сайта и маскированный секрет. Поиск по комментарию, логину и URL; копирование по ПКМ. В общей истории этих строк **нет**.
...
| **Пароли** | Явно сохранённые секреты: комментарий, логин, URL, `••••••••`. |

Избранное и сохранение в «Пароли» — через **контекстное меню** (ПКМ): добавить/убрать из избранного, сохранить в пароли, копировать логин/URL/пароль, открыть URL в браузере, редактировать, удалить.

### Поиск, превью, вставка

- **Поиск** (⌘\\) — нечёткое сопоставление по тексту; для паролей — по комментарию, логину и URL, не по секрету.
...
Интерфейс на **всех языках macOS** (`Localizable.xcstrings`); язык следует системной локали.
...
Нужно: **Xcode**, **macOS 10.15+** (зависимости подтягиваются через Swift Package Manager при сборке).
...
2. **Открыть** `VaultClip.xcodeproj` в Xcode (первая сборка загрузит SPM-пакеты: RxSwift, HotKey, Fuse).
3. **Схемы Xcode:**
...
Форк [Yippy](https://github.com/mattDavo/Yippy) (Matthew Davidson), изначально MIT. К VaultClip в целом применяется GPLv3. Сторонний код (RxSwift, HotKey, Fuse, вендоренные Default и LoginServiceKit) — под своими лицензиями.
