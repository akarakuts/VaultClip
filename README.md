...
Saved entries show a comment, login, optional site URL, and masked secret. Search by comment, login, and URL; copy via the context menu. These rows **do not appear** in general history.
...
| **Passwords** | Explicitly saved secrets: comment, login, URL, `••••••••`. |

Favorites and saving to Passwords are available from the **context menu** (right-click): add/remove favorite, save to passwords, copy login/URL/password, open URL in browser, edit, delete.

### Search, preview, paste

- **Search** (⌘\\) — fuzzy text matching; for passwords, search runs on comment, login, and URL, not the secret.
...
UI in **all macOS app languages** (`Localizable.xcstrings`); the active language follows the system locale.
...
Requirements: **Xcode**, **macOS 10.15+** (Swift Package Manager resolves dependencies at build time).
...
2. **Open** `VaultClip.xcodeproj` in Xcode (first build resolves SPM packages: RxSwift, HotKey, Fuse).
3. **Xcode schemes:**
...
```bash
xcodebuild -project VaultClip.xcodeproj -scheme VaultClip -configuration Release \
...
Fork of [Yippy](https://github.com/mattDavo/Yippy) (Matthew Davidson), originally MIT. GPLv3 applies to VaultClip as a whole. Third-party code (RxSwift, HotKey, Fuse, vendored Default and LoginServiceKit) remains under its respective licenses.
